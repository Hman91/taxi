...
import '../widgets/maps/taxi_map_launch_card.dart';
...
class _AppPassengerScreenState extends State<AppPassengerScreen> {
  ...
  Widget _appBarHomeLogo() => GestureDetector(
        onTap: () {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (_) =>
                  const UnifiedLoginScreen(showPassengerEntry: true),
            ),
          );
        },
  ...
  Future<void> _requestRide() async {
    ...
    final preferredCurrentTourist =
        availableOptions.contains(_TripOptionType.currentToTourist);
    final preferredCurrentAirport =
        availableOptions.contains(_TripOptionType.currentToAirport);
    ...
    var quotePrimed = false;

    String durationLabelFromSeconds(int sec) {
      if (sec < 3600) return '${(sec / 60).ceil()} min';
      final h = sec ~/ 3600;
      final m = (sec % 3600) ~/ 60;
    ...
    Future<void> recalcQuote(StateSetter ss) async {
      ...
      if (origin != null && _directions.isConfigured) {
        final parts = selectedRouteKey.split(airportRouteKeySeparator);
        if (parts.length >= 2) {
          final destZone = parts[1].trim();
          final dest = TunisiaZoneCoordinates.lookup(destZone);
          if (dest != null) {
            final route = await _directions.fetchRoute(origin, dest);
            if (route != null &&
                route.points.length >= 2 &&
                route.distanceMeters > 0) {
              try {
                var q = await _api.quoteGps(distanceKm: route.distanceKm);
                var fare = (q['final_fare'] as num?)?.toDouble() ?? 0;
                if (promoCtrl.text.trim() == 'WELCOME26') fare *= 0.8;
                q = Map<String, dynamic>.from(q);
                q['final_fare'] = double.parse(fare.toStringAsFixed(3));
                q['route_key'] = selectedRouteKey;
                q['quote_mode'] = 'gps';
                q['directions_km'] = route.distanceKm;
                q['directions_duration_seconds'] = route.durationSeconds;
                ss(() => quote = q);
                return;
              } catch (_) {}
            }
      ...
      try {
        ...
        var fare = (q['final_fare'] as num?)?.toDouble() ??
            (q['base_fare'] as num?)?.toDouble() ??
            (_fares[selectedRouteKey] ?? 0);
        if (promoCtrl.text.trim() == 'WELCOME26') fare *= 0.8;
        final qm = Map<String, dynamic>.from(q);
        qm['final_fare'] = double.parse(fare.toStringAsFixed(3));
        qm['route_key'] = selectedRouteKey;
        qm['quote_mode'] = 'airport';
        ss(() => quote = qm);
      } catch (_) {
        ss(() => quote = null);
      }
                    ...
                    if (quote != null) ...[
                      Container(
                        ...
                        child: Column(children: [
                          ...
                          if (quote!['quote_mode'] == 'gps' &&
                              quote!['directions_km'] != null &&
                              quote!['directions_duration_seconds'] !=
                                  null) ...[
                            const SizedBox(height: 12),
                            Wrap(
                              ...
                              children: [
                                Container(
                                  ...
                                  child: Text(
                                    '${(quote!['directions_km'] as num).toStringAsFixed(1)} km',
                                    style: const TextStyle(
                                        fontWeight: FontWeight.w800,
                                        fontSize: 11,
                                        color: _C.charcoal),
                            ...
                            Text(
                              ...
                              style: TextStyle(
                                ...
                                fontWeight: FontWeight.w900,
                                letterSpacing: 1.1,
                                color: _C.textSoft.withValues(alpha: 0.9),
                              ),
                            ),
                          ],
                        ]),
        ...
        children: [
          if (_token == null) ...[
            ...
            _TaxiCard(
                child: Column(
                  ...
                  TextField(
                      ...
                      decoration:
                          _fd('Passenger name', icon: Icons.badge_outlined)),
                  const SizedBox(height: 10),
                  TextField(
                  ...
                  TextField(
                      ...
                      decoration: _fd(l.operatorPhoneLabel,
                  ...
                  TextField(
                    ...
                    decoration:
                        _fd(l.passwordLabel, icon: Icons.lock_outline_rounded)
                            .copyWith(
                      suffixIcon: IconButton(
                        onPressed: () => setState(() =>
                  ...
                  if (_signupPhotoData.isNotEmpty) ...[
                    ...
                    ClipRRect(
                        ...
                        child: Builder(builder: (_) {
                          ...
                          return p != null
                              ? Image(image: p, height: 90, fit: BoxFit.cover)
                              : const SizedBox.shrink();
                        })),
                    TextButton.icon(
                        onPressed: () => setState(() => _signupPhotoData = ''),
            ...
            AnimatedContainer(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('Your premium ride, ready when you are.',
                      style: TextStyle(
                          color: _C.charcoal,
                          fontSize: 25,
                          height: 1.08,
                  ...
                  Row(
                    children: [
                      Expanded(
                          child: _statBadge(
                              '$activeCount ${l.passengerActiveRidesChip(activeCount).split(' ').last}',
                              _C.neonBlue)),
                      const SizedBox(width: 10),
                      Expanded(
                          child:
                              _statBadge('${_rides.length} total', _C.yellow)),
                    ],
                  ),
                ],
              ),
            ...
            TaxiMapLaunchCard(
              ...
              subtitle:
                  'Places search, route preview, pickup & destination — synced with your active ride when available.',
              onOpen: _openPassengerLiveMap,
            ),
            // Book
            _TaxiCard(
                accent: true,
                      ...
                      Row(children: [
                        ...
                        Expanded(
                            child: Column(
                              ...
                              const Text(
                                  'Airport transfer • instant or scheduled',
                                  style: TextStyle(
                                      color: _C.textSoft,
                                      fontSize: 12,
                                      fontWeight: FontWeight.w700)),
  ...
  Widget _statBadge(String label, Color bg) => Container(
        ...
        child: Text(label,
            style: TextStyle(
                color: bg == _C.charcoal ? _C.charcoal : bg,
                fontSize: 11,
                fontWeight: FontWeight.w700)),
      );
            ...
            Expanded(
                child: Column(
                  ...
                  Container(
                    ...
                    child: Text(localizedRideStatusLabel(l, r.status),
                        style: TextStyle(
                            color: sc,
                            fontSize: 10,
                            fontWeight: FontWeight.w800)),
                    ...
                    if ((r.driverName ?? '').isNotEmpty)
                      Row(children: [
                        const Icon(Icons.person_outline_rounded,
                            color: _C.textSoft, size: 13),
                        const SizedBox(width: 5),
                        Text(l.passengerDriverLine(r.driverName!),
                            style: const TextStyle(
